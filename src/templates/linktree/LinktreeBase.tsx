...
interface LinktreeBaseProps {
  data: LinktreeData;
  templateName: string;
}

export function LinktreeBase({ data, templateName }: LinktreeBaseProps) {
    ...
    const ctx = gsap.context(() => {
      if (avatarRef.current) {
        gsap.fromTo(avatarRef.current,
          ...
          { scale: 1, rotate: 0, opacity: 1, duration: 0.8 * speed, ease: 'back.out(1.7)' }
        );
